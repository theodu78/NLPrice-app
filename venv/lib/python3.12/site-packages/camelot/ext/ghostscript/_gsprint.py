...
def set_stdio(instance, stdin, stdout, stderr):
    ...
    You may pass ``None`` for any of stdin, stdout ou stderr, auquel cas le système stdin, stdout resp. stderr sera utilisé.
    """
...
def init_with_args(instance, argv):
    ...
    1. Si quitter ou EOF se produit pendant init_with_args(), la valeur de retour
       sera e_Quit. Ce n'est pas une erreur. Vous devez appeler exit() et
       ne doit pas appeler d'autres fonctions.

    2. Si des informations d'utilisation doivent être affichées, la valeur de retour sera
       e_Info, ce qui n'est pas une erreur. Ne pas appeler exit().

    3. Dans des conditions normales, cette fonction renvoie 0. Vous appelleriez ensuite un ou plusieurs
       fonctions run_*() puis terminez avec exit()
    """
...
def exit(instance):
    ...
    Cela doit être appelé à l'arrêt si init_with_args() a été
    appelé, et juste avant delete_instance()
    """
...
def __win32_finddll():
    ...
    dlls = []
    # Recherchez différentes variantes de Ghostscript et prenez la plus élevée
    # version pour laquelle la DLL se trouve dans le système de fichiers.
    for key_name in (
        "AFPL Ghostscript",
        "Aladdin Ghostscript",
        "GNU Ghostscript",
...
if sys.platform == "win32":
    ...
    if not libgs:
        raise RuntimeError("Veuillez vous assurer que Ghostscript est installé")
    libgs = windll.LoadLibrary(libgs)
else:
    try:
        libgs = cdll.LoadLibrary("/opt/homebrew/lib/libgs.dylib")  # Chemin complet vers libgs.dylib
    except OSError:
